...
with open(DB_DIR / "cinema.json", encoding="utf-8") as f:
    data = json.load(f)
...
with open(DB_DIR / "filmes.json", "w", encoding="utf-8") as f:
    json.dump(filmes, f, ensure_ascii=False, indent=2)

with open(DB_DIR / "atores.json", "w", encoding="utf-8") as f:
    json.dump(atores, f, ensure_ascii=False, indent=2)

with open(DB_DIR / "generos.json", "w", encoding="utf-8") as f:
    json.dump(generos, f, ensure_ascii=False, indent=2)

print(f"✔ filmes.json  → {len(filmes)} documentos")
print(f"✔ atores.json  → {len(atores)} documentos")
print(f"✔ generos.json → {len(generos)} documentos")
